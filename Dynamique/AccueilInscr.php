            ...
            if ($pseudoexist == 0) {
                ...
                if ($prenomlen <= 50 || $nomlen <= 50) {
                    ...
                    if (filter_var($mail, FILTER_VALIDATE_EMAIL)) {
                        $reqmail = $db->prepare('SELECT * FROM utilisateur WHERE email="' . $mail . '"');
                        $reqmail->execute();
                        ...
                        if ($mailexist == 0) {
                            ...
                            if ($mdp == $mdpc) {
                                $reponse = $db->prepare("INSERT INTO utilisateur(pseudo,nom,prenom,age,date,pays,adresse,sexe,email,mdp) 
                                                                        values ('$pseudo','$nom', '$prenom','$age','$date','$pays','$adrs','$sexe','$mail','$mdp')");
                                $reponse->execute();
                                $error = 'Votre compte a été crée !';
                                header('Location: http://localhost:63342/Testir/Website/Dynamique/PageAccueil.php?_ijt=it7vdcji0lbpeit9iad1om4u7j');
